...
public class LedgeClimbing : MonoBehaviour
{
	...
	private RaycastHit climbingPoint;
	...
	private void Update()
	{
		if (!Player.isGrounded && !objInt.holdingObject && !objInt.throwingObject && !isClimbing)
			CheckLedge();
	...
	private void CheckLedge()
	{
		...
		if (Physics.SphereCast(transform.position, ledgeSphereCastRadius, Vector3.up + transform.forward, out ledgeHit, ledgeDetectionLength, ledgeLayer))
		{
			ledgeDetected = true;

			if (Physics.Raycast(ledgeHit.point + (cam.transform.forward * forwardClimb) + (upClimb * Vector3.up), Vector3.down, out climbingPoint, Player.controller.height))
			{
				if (Player.targetDir.y == 1 && Player.jumpHeld)
				{
